...
data = np.zeros((len(ch_names), 200)) # 2 seconds long epochs
...
N = raw._data.shape[0]
T = raw._data.shape[1]//2 # first half of the data as baseline
...
for sim_num in range(10):
    X_raw = np.zeros((S*O, T*2))
    EX_raw = np.random.randn(S*O, T*2) # noise applied in source locations
    E_raw = np.random.randn(N, T*2) # noise applied in sensor locations
    ...
    # generate two active signals
    for start_idx in range(len(selected_locs)):
        start = selected_locs[start_idx]
        amp = AR_series(5,T) # generate AR(5) series as signal amplitude
        ori = np.random.randn(O)
        ori = ori/np.linalg.norm(ori) # generate random orientation
        X_raw[(start*O):(start*O+O), (T):] = np.outer(ori, amp)*1e-7 # in MNE, the data unit is volt for EEG
    ...
    X_raw_backgroud = np.zeros((S*O, T*2))
    ...
    X_raw_min = np.linalg.norm(X_raw[selected_locs_full,T:].reshape(dipole_num,O,T), axis = (1,2))
    X_raw_min = X_raw_min.min()

    # the maximum signal energy in the background sources
    X_raw_backgroud_max = np.linalg.norm(X_raw_backgroud[:,T:].reshape(S,O,T), axis = (1,2))
    ...
    energy_X = np.linalg.norm(X_raw)**2 / (X_raw.shape[1] * X_raw.shape[0])
    energy_EX = np.linalg.norm(EX_raw)**2 / (EX_raw.shape[1] * EX_raw.shape[0])
    ...
    for snr_s in [20, 10]:
        ...
        Xtrue = X[selected_locs_full,T:].reshape((dipole_num, O, T))
        ...
        energy_Y = np.linalg.norm(Y_raw)**2 / (Y_raw.shape[1] * Y_raw.shape[0])
        energy_E = np.linalg.norm(E_raw)**2 / (E_raw.shape[1] * E_raw.shape[0])

        for snr_n in [20, 10, 0]:
            # scale the noise level
            ...
            cov = mne.compute_covariance(epochs, method='empirical', tmin=0, tmax=0.99)
            # compute noise+signal covariance matrix
            cov_all = mne.compute_covariance(epochs, method='empirical', tmin=1, tmax=1.99)

            # generate the evoked data
            evoked = epochs.average()

            # plot the evoked data
            evoked = evoked.crop(0.75,1.99)
            fig, axs = plt.subplots(1, 1, figsize=(7,3.7))
            evoked.plot(axes = axs,spatial_colors=True,highlight=np.array([1.0,1.99]),show = False,
                        ylim = {'eeg':[-100, 100]},
            ...
            for mymethod in ['DeESI']:
                if mymethod == 'DeESI':
                    ...
                    for lambda_i in [1,0.75,0.5,0.4,0.3,0.2,0.1,0.05]:
                        ...
                        Xout_debias_rotate = Xout_debias_rotate.reshape((S,O,T))
